...
// Check if donor ID is set in the session
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sid'])) {
    ...
    // Update the donation status and donated blood pint for the specific donor and seeker
    $update_query = "UPDATE blooddonationrequests SET donation_status = ?, donated_blood_pint = ? WHERE sid = ? AND drid = ?";
    $update_statement = $conn->prepare($update_query);
    $update_statement->bind_param("sisi", $donation_status, $donated_blood_pint, $sid, $drid);
    ...
    if ($update_statement->execute()) {
        ...
        // Set success message
        $_SESSION['success_message'] = "Details updated and stored in the database successfully.";
    } else {
        // Handle any errors that may occur during the update
        $_SESSION['error_message'] = "Error updating details: " . $update_statement->error;
    }
    $update_statement->close();
}

// Fetch the accepted donation requests for the specific donor
        ...
        <?php if (isset($_SESSION['error_message'])): ?>
            <div class="alert alert-danger" role="alert">
                <?php 
                    ...
                    unset($_SESSION['error_message']); // Unset the message after displaying it
                ?>
            </div>
        <?php endif; ?>
            ...
            <div class="modal-body">
                <form action="" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="donated_blood_pint" name="donated_blood_pint" value="<?php echo htmlspecialchars($row['donated_blood_pint']); ?>" min="0">
                    </div>
